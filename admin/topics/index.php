...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_bulk']) && isset($_POST['bulk_action'])) {
    if (!isset($_SESSION['admin']) || !$_SESSION['admin']) { header('location: ' . BASE_URL); exit; }
    $ids = $_POST['selected_ids'] ?? [];
    $action = $_POST['bulk_action'];
    if (!empty($ids)) {
        foreach ($ids as $tid) {
            $tid = (int)$tid;
            if ($action === 'delete') { delete('topics', $tid); }
        }
    }
...
<head>
    ...
    <title>Админ — Категории | AvtoTachka</title>
</head>
<body>
    ...
    <div class="container">
        ...
        <div class="col-9">
            ...
            <a href="<?php echo BASE_URL; ?>admin/topics/create.php" class="btn btn-success mb-3"><i class="fas fa-plus"></i> Добавить категорию</a>
            <form action="index.php" method="post" id="bulkForm">
                ...
                <table class="table table-hover">
                    <thead><tr><th><input type="checkbox" id="selectAll"></th><th>ID</th><th>Название</th><th>Действия</th></tr></thead>
                    <tbody>
                        <?php foreach ($topics as $topic): ?>
                        <tr>
                            <td><input type="checkbox" name="selected_ids[]" value="<?= $topic['id'] ?>" class="rowCheckbox"></td>
                            <td><?= $topic['id'] ?></td>
                            <td><?= htmlspecialchars($topic['name']) ?></td>
                            <td>
                                <a href="edit.php?id=<?= $topic['id'] ?>" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a>
                                <a href="javascript:void(0)" data-href="index.php?del_id=<?= $topic['id'] ?>" class="btn btn-sm btn-danger delete-btn"><i class="fas fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
