...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_bulk'])) {
    ...
    if (!empty($ids) && $_POST['bulk_action'] === 'delete') {
        foreach ($ids as $bid) { delete('brands', (int)$bid); }
    }
    header('location: ' . BASE_URL . 'admin/brands/index.php'); exit;
}
if (isset($_GET['del_id'])) {
    delete('brands', (int)$_GET['del_id']);
    header('location: ' . BASE_URL . 'admin/brands/index.php'); exit;
}
...
$where   = $search ? "WHERE name LIKE :s OR country LIKE :s2" : "";
$params  = $search ? [':s'=>"%$search%",':s2'=>"%$search%"] : [];

$cnt = $pdo->prepare("SELECT COUNT(*) FROM brands $where");
$cnt->execute($params);
$totalCount = (int)$cnt->fetchColumn();
$totalPages = max(1, ceil($totalCount / $perPage));

$stmt = $pdo->prepare("SELECT * FROM brands $where ORDER BY name ASC LIMIT :lim OFFSET :off");
...
<div class="container">
    ...
    <div class="col-9">
        ...
        <div class="d-flex gap-2 mb-3 align-items-center flex-wrap">
            <a href="<?php echo BASE_URL; ?>admin/brands/create.php" class="btn btn-success"><i class="fas fa-plus"></i> Добавить бренд</a>
            <form method="get" action="" class="d-flex gap-2 flex-grow-1" style="max-width:420px">
                <input type="text" name="search" class="form-control" placeholder="?? Название или страна..." value="<?= htmlspecialchars($search) ?>">
        ...
        <form action="index.php<?= $search ? '?search='.urlencode($search) : '' ?>" method="post" id="bulkForm">
            ...
            <table class="table table-hover">
                <thead><tr><th><input type="checkbox" id="selectAll"></th><th>ID</th><th>Логотип</th><th>Название</th><th>Страна</th><th>Действия</th></tr></thead>
                <tbody>
                <?php foreach ($brands as $b): ?>
                <tr>
                    <td><input type="checkbox" name="selected_ids[]" value="<?= $b['id'] ?>" class="rowCheckbox"></td>
                    <td><?= $b['id'] ?></td>
                    <td><?php if ($b['logo']): ?><img src="<?= BASE_URL ?>assets/images/brands/<?= $b['logo'] ?>" width="40" alt=""><?php else: ?><i class="fas fa-car"></i><?php endif; ?></td>
                    <td><?= htmlspecialchars($b['name']) ?></td>
                    <td><?= htmlspecialchars($b['country'] ?? '') ?></td>
                    <td>
                        <a href="<?= BASE_URL ?>admin/brands/edit.php?id=<?= $b['id'] ?>" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a>
                        <a href="javascript:void(0)" data-href="<?= BASE_URL ?>admin/brands/index.php?del_id=<?= $b['id'] ?>" class="btn btn-sm btn-danger delete-btn"><i class="fas fa-trash"></i></a>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php if (empty($brands)): ?><tr><td colspan="6" class="text-center text-muted py-4">Ничего не найдено</td></tr><?php endif; ?>
                </tbody>
            </table>
        </form>
        <?php if ($totalPages > 1): ?>
...
<div id="deleteOverlay" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:999999;justify-content:center;align-items:center;">
  <div style="background:#0a0a0a;border:2px solid #0f0;border-radius:8px;padding:30px;min-width:320px;text-align:center;color:#0f0;">
    ...
    <p style="margin:0 0 25px;color:#0f0;">Удалить бренд?</p>
